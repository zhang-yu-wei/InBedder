...
class MTEB:
    ...
    def robustness_test(
        ...
        model,
        verbosity=1,
        output_folder="robustness_results/result",
        eval_splits=None,
        overwrite_results=False,
        raise_error: bool = True,
        **kwargs
        ...
        """
        Run the robustness evaluation pipeline on the selected tasks. This will start a for-loop over all possible instructions.
        ...
        while len(self.tasks) > 0:
            ...
            try:
                ...
                for split in task_eval_splits:
                    # evaluate on correct instructions
                    task_results[split] = {"correct_instructions": [],
                                           "implicit_instructions": [],
                                           "incorrect_instructions": []}
                    cache_dir = kwargs.get('cache_dir', None)
                    for inst_k in task_results[split]:
                        for i, instruction in enumerate(task.description.get(inst_k, [])):
                            if cache_dir is not None:
                                kwargs['cache_dir'] = os.path.join(cache_dir, f"{inst_k}_{i}")
                            tick = time()
                            results = task.evaluate(model, split, instruction=instruction, **kwargs)
                            tock = time()
                            logger.info(f"Evaluation for {task.description['name']} on {split} took {tock - tick:.2f} seconds")
                            results["evaluation_time"] = round(tock - tick, 2)
                            task_results[split][inst_k].append(results[main_score])
                            if verbosity >= 1:
                                logger.info(f"Scores: {results}")
                    correct_min = min(task_results[split]["correct_instructions"])
                    implicit_min = min(task_results[split]["implicit_instructions"])
                    incorrect_max = max(task_results[split]["incorrect_instructions"])
                    task_results[split]["dci"] = correct_min - incorrect_max
                    task_results[split]["dii"] = implicit_min - incorrect_max
