...
class RerankingEvaluator(Evaluator):
    ...
    def compute_metrics_batched(self, model):
        ...
        if self.cache_dir is not None:
            encode_params["cache_dir"] = os.path.join(self.cache_dir, "queries")
        ...
        if isinstance(self.samples[0]["query"], str):
            ...
            all_query_embs = np.asarray(encode_queries_func(all_query_flattened, **encode_params))
        else:
        ...
        if self.cache_dir is not None:
            encode_params["cache_dir"] = os.path.join(self.cache_dir, 'corpus')
        all_docs_embs = np.asarray(encode_corpus_func(all_docs, **encode_params))
