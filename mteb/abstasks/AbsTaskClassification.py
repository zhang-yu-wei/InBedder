    ...
    def evaluate(self, model, eval_split="test", train_split="train", **kwargs):
        ...
        if self.is_multilingual:
            ...
            for lang in self.dataset:
                ...
                scores[lang] = self._evaluate_monolingual(model, self.dataset[lang], lang, eval_split, train_split, **kwargs)
                self._add_main_score(scores[lang])
        else:
            logger.info(f"\nTask: {self.description['name']}, split: {eval_split}. Running...")
            scores = self._evaluate_monolingual(model, self.dataset, None, eval_split, train_split, **kwargs)
            self._add_main_score(scores)

        return scores

    def _evaluate_monolingual(self, model, dataset, lang=None, eval_split="test", train_split="train", **kwargs):
        disable_instruction = kwargs['disable_instruction']
        pattern = kwargs['pattern']
        train_split = dataset[train_split]
        eval_split = dataset[eval_split]
        eval_text_instruct = self._add_instruction(pattern, eval_split['text'], lang)
        ...
        for i in range(self.n_experiments):
            ...
            X_sampled_instruct = self._add_instruction(X_sampled, lang)
            ...
            if disable_instruction:
                ...
                if self.method == "kNN":
                    evaluator = kNNClassificationEvaluator(
                        X_sampled_instruct, y_sampled, eval_text_instruct, eval_split["label"], **params
                    )
                elif self.method == "kNN-pytorch":
                    evaluator = kNNClassificationEvaluatorPytorch(
                        X_sampled_instruct, y_sampled, eval_text_instruct, eval_split["label"], **params
