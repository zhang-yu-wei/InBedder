"""
Re-purpose Seq2SeqLM for encoding representations
"""
...
SEQ2SEQ_OUTPUTVALUES = ["avg_gen_layer_", "avg_ppt_layer_", "last_gen_layer_", "first_gen_layer_", "re_encode"]

def verify_output_value(output_value: str):
    for ov in SEQ2SEQ_OUTPUTVALUES:
        if output_value.startswith(ov):
            return True
    return False

class Seq2SeqLMEncoder:
    """implement convenient encode function that has similar appearance as sentence-transformers"""
    def __init__(self, model_name_or_path: str,
                tokenizer_path: str = None,
                output_value: str = "avg_gen_layer_1",
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, padding_side='right', truncation_side="right", use_fast=False)

        logger.info(f"Use dtype {dtype}")
        logger.info(f"Loading model from {model_name_or_path} ...")
        # seqseq architectures currently do not support flash attention 2
        self.model = AutoModelForSeq2SeqLM.from_pretrained(model_name_or_path,
                                                           torch_dtype=torch.bfloat16 if dtype == "bfloat16" else torch.float32,
                                                           low_cpu_mem_usage=True,
                                                           trust_remote_code=True,
                                                           device_map="auto")
        assert verify_output_value(output_value), f"{output_value} is not defined, please choose from {SEQ2SEQ_OUTPUTVALUES}"
    ...
    def get_all_output_values(self) -> List[str]:
        # +1 token embedding
        num_encoder_layers = self.model.config.num_layers + 1
        num_decoder_layers = self.model.config.num_decoder_layers + 1
        all_output_values = []
        for ov in SEQ2SEQ_OUTPUTVALUES:
            if ov.endswith("_") and 'gen' not in ov:
                all_output_values.extend([ov+str(l) for l in range(num_encoder_layers)])
            elif ov.endswith("_"):
                all_output_values.extend([ov+str(l) for l in range(num_decoder_layers)])
            else:
                all_output_values.append(ov)
        return all_output_values
    
    def get_last_output_values(self) -> List[str]:
        return [ov + str(self.model.config.num_hidden_layers) if ov.endswith("_") else ov for ov in SEQ2SEQ_OUTPUTVALUES]
    ...
    def encode(self, sentences: Union[str, List[str]],
        ...
        if all_embeddings is None or all_generations is None:
            ...
            for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
                ...
                all_generations += [st + g for st, g in zip(sentences_batch, generations)]

                encoder_hidden_states = outputs["encoder_hidden_states"]
                decoder_hidden_states = outputs["decoder_hidden_states"]

                aggregated = self._aggregate_embeddings(encoder_hidden_states, decoder_hidden_states, features['attention_mask'], generations, outputs['sequences'], filter_ids)
                for k in aggregated:
    ...
    def _aggregate_embeddings(self, encoder_hidden_states, decoder_hidden_states, attention_mask, generations, decoded_ids, filter_ids):
        """
        ...
        prompt_hidden_states = encoder_hidden_states

        # might not be same layers for encoder and decoder
        if self.last_layer_only:
            encoder_layer_ids = [len(prompt_hidden_states) - 1]
        else:
            encoder_layer_ids = list(range(len(prompt_hidden_states)))
        for encoder_layer_id in encoder_layer_ids:
            ...
            embeddings["avg_ppt_layer_" + str(encoder_layer_id)] = []
            for batch_id in range(len(prompt_hidden_states[encoder_layer_id])):
                prompt_hidden_states_remove_pad = prompt_hidden_states[encoder_layer_id][batch_id, attention_mask[batch_id].bool(), :]
                embeddings["avg_ppt_layer_" + str(encoder_layer_id)].append(prompt_hidden_states_remove_pad.mean(0).cpu())
            embeddings["avg_ppt_layer_" + str(encoder_layer_id)] = torch.stack(embeddings["avg_ppt_layer_" + str(encoder_layer_id)], dim=0)

        # post-process to remove special tokens in decoded sequences
        # decoding starts from second token, because the first token is BOS
        ...
        if self.last_layer_only:
            decoder_layer_ids = [len(decoder_hidden_states[0]) - 1]
        else:
            decoder_layer_ids = list(range(len(decoder_hidden_states[0])))
        for decoder_layer_id in decoder_layer_ids:
            embeddings["avg_gen_layer_" + str(decoder_layer_id)] = []
            embeddings["last_gen_layer_" + str(decoder_layer_id)] = []
            embeddings["first_gen_layer_" + str(decoder_layer_id)] = []
            for batch_id in range(len(decoded_ids)):
                special_tokens_mask = special_tokens_masks[batch_id]
                if sum(special_tokens_mask) < len(special_tokens_mask):
                    generation_hidden_states = torch.cat([hs[decoder_layer_id][batch_id] for hs, special_token in zip(decoder_hidden_states, special_tokens_mask) if not special_token], dim=0)
                else:
                    # if all of them are special token, then there is a problem
                    generation_hidden_states = decoder_hidden_states[0][decoder_layer_id][batch_id]
                embeddings["avg_gen_layer_" + str(decoder_layer_id)].append(generation_hidden_states.mean(0).cpu())
                embeddings["last_gen_layer_" + str(decoder_layer_id)].append(generation_hidden_states[-1, :].cpu())
                embeddings["first_gen_layer_" + str(decoder_layer_id)].append(generation_hidden_states[0, :].cpu())
            embeddings["avg_gen_layer_" + str(decoder_layer_id)] = torch.stack(embeddings["avg_gen_layer_" + str(decoder_layer_id)], dim=0)
            embeddings["last_gen_layer_" + str(decoder_layer_id)] = torch.stack(embeddings["last_gen_layer_" + str(decoder_layer_id)], dim=0)
            embeddings["first_gen_layer_" + str(decoder_layer_id)] = torch.stack(embeddings["first_gen_layer_" + str(decoder_layer_id)], dim=0)
