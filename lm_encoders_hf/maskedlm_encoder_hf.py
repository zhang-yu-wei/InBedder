"""
Re-purpose MaskedLMs for encoding representations
"""
...
MASKED_OUTPUTVALUES = ["avg_gen_layer_", "avg_ppt_layer_", "avg_all_layer_", "re_encode"]

def verify_output_value(output_value: str):
    for ov in MASKED_OUTPUTVALUES:
        if output_value.startswith(ov):
            return True
    return False

class MaskededLMEncoder:
    ...
    def __init__(self, model_name_or_path: str,
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, padding_side='right', truncation_side="left")
        ...
        assert verify_output_value(output_value), f"{output_value} is not defined, please choose from {MASKED_OUTPUTVALUES}"
    ...
    def get_last_output_values(self) -> List[str]:
        return [ov + str(self.model.config.num_hidden_layers) if ov.endswith("_") else ov for ov in MASKED_OUTPUTVALUES]
    ...
    def encode(self, sentences: Union[str, List[str]],
        ...
        if all_embeddings is None or all_generations is None:
            ...
            for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
                ...
                sentences_batch = [st + self.tokenizer.mask_token * self.generation_configs['mask_length'] for st in sentences_batch]
                ...
                outputs = self.model(**features, return_dict=True, output_hidden_states=True)
                logits = outputs["logits"]
                sequences = torch.argmax(logits, dim=-1)
                generations = self.tokenizer.batch_decode(sequences, skip_special_tokens=True)
                all_generations += generations

                logits_masked = logits[torch.eq(features["input_ids"], self.tokenizer.mask_token_id), :]
                logits_masked = logits_masked.view(len(sentences_batch), -1, logits.shape[-1])
                sequences_masked = torch.argmax(logits_masked, dim=-1)
                generations_masked = self.tokenizer.batch_decode(sequences_masked, skip_special_tokens=True)

                hidden_states = outputs["hidden_states"]

                aggregated = self._aggregate_embeddings(hidden_states, features['input_ids'], generations_masked)
                for k in aggregated:
    ...
    def _aggregate_embeddings(self, hidden_states, input_ids, generations):
        """
        ...
        if self.last_layer_only:
            layer_ids = [len(hidden_states) - 1]
        else:
            layer_ids = list(range(len(hidden_states)))
        for layer_id in layer_ids:
            ...
            for batch_id in range(len(hidden_states[layer_id])):
                hs = hidden_states[layer_id][batch_id]
                
                not_mask = torch.ne(input_ids[batch_id], self.tokenizer.mask_token_id)
                not_pad = torch.ne(input_ids[batch_id], self.tokenizer.pad_token_id)
                not_eos = torch.ne(input_ids[batch_id], self.tokenizer.eos_token_id)
                not_mask_pad_eos = torch.logical_and(not_mask, not_pad)
                not_mask_pad_eos = torch.logical_and(not_mask_pad_eos, not_eos)
                
                prompt_hs = hs[not_mask_pad_eos, :].mean(0).cpu()
                embeddings["avg_ppt_layer_" + str(layer_id)].append(prompt_hs)

                is_mask = torch.eq(input_ids[batch_id], self.tokenizer.mask_token_id)
                generation_hs = hs[is_mask, :].mean(0).cpu()
                embeddings["avg_gen_layer_" + str(layer_id)].append(generation_hs)

                all_hs = hs[not_pad, :].mean(0).cpu()
                embeddings["avg_all_layer_" + str(layer_id)].append(all_hs)
